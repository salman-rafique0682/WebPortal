...
@Component({
    ...
    template: `
        ...
        >
            <kanban-list *ngFor="let list of lists; let i = index" [list]="list" [listIds]="listIds" cdkDrag
                         cdkDragHandle [cdkDragDisabled]="isMobileDevice" class="p-kanban-list"></kanban-list>
            <div class="px-4 py-1 mb-4 md:w-[25rem] shrink-0">
                <button pButton pRipple label="New List" icon="pi pi-plus font-semibold"
                        class="py-4 justify-center font-semibold w-full rounded" (click)="addList()"></button>
            </div>
