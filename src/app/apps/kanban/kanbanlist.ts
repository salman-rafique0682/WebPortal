...
@Component({
    ...
    template: `
            ...
            <p-inplace>
                ...
                <ng-template #content let-closeCallback="closeCallback">
                    <span class="inline-flex items-center gap-1 h-10">
                        <input type="text" autofocus pInputText [value]="list.title" [(ngModel)]="list.title"
                               class="h-full !rounded-r-none w-[10rem]"/>
                        <button (click)="closeCallback($event)" pButton icon="pi pi-check"
                                class="!rounded-l-none !h-10"></button>
                    </span>
                </ng-template>
            </p-inplace>
            <div>
        ...
        >
            <kanban-card *ngFor="let card of list.cards" [card]="card" [listId]="list.listId"
                         (click)="onCardClick($event, card)" cdkDrag [cdkDragDisabled]="isMobileDevice"></kanban-card>
        </div>
        <div class="px-6 mb-4 w-full mt-6 flex">
            <button pButton pRipple label="New Card" icon="pi pi-plus font-semibold"
                    class="py-4 justify-center font-semibold w-full rounded-border" (click)="insertCard()"></button>
        </div>
    `,
    host: {
